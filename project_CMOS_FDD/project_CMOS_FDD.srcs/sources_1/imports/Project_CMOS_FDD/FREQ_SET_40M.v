...
/*-------               Main Code            --------*/
/*****************************************************/
// Reference clock(40MHz Xtal) --> [Ref divider(x2)] --> F_ref(80MHz) --> [PLL(N_integer & N_fractional)] --> F_rfpll --> [VCO devider] --> LO(46.875MHz-6GHz)
...
// Calculate N_integer according to Pll input(F_ref = 80MHz) and  Pll output(F_rfpll, which is also the VCO input = LO * 2^( VCO divider + 1 ) )
...
// Rx_dividend1_sub = ( F_rfpll / F_ref  - N_integer ) *40MHz, which is actually some form of fix_point of ( F_rfpll / F_ref  - N_integer )
always @ (posedge Clk)
begin
    case(Rx_VcoDivider)
...
begin
    ...
    else if( Rx_dividend1_sub >= 26'd40000000 )
        begin
            Rx_dividend1_sub <= Rx_dividend1_sub - 26'd40000000;
...
begin
    ...
    else if( Tx_dividend1_sub >= 26'd40000000 )
        begin
            Tx_dividend1_sub <= Tx_dividend1_sub - 26'd40000000;
...
begin
    ...
    else if( Rx_dividend2_sub >= 26'd40000000 )
        begin
            Rx_dividend2_sub <= Rx_dividend2_sub - 26'd40000000;
            Rx_sub2_cnt      <= Rx_sub2_cnt + 1'b1;
        end
    else if( Rx_dividend2_sub < 25'd20000000 )
        Rx_rounding <= 1'b0;
    else
        ;
end
...
begin
    ...
    else if( Tx_dividend2_sub >= 26'd40000000 )
        begin
            Tx_dividend2_sub <= Tx_dividend2_sub - 26'd40000000;
            Tx_sub2_cnt      <= Tx_sub2_cnt + 1'b1;
        end
    else if( Tx_dividend2_sub < 25'd20000000 )
        Tx_rounding <= 1'b0;
    else
        ;
end
...
always @ (posedge Clk)
begin
    if( ( Rx_dividend2_sub < 26'd40000000 ) && ( Tx_dividend2_sub < 26'd40000000 ) )
        Cal_end <= 1'b1;
    else
        Cal_end <= 1'b0;;
end
