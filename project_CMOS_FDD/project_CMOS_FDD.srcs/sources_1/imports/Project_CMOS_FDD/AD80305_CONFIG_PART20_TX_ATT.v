/************************************************************
module name 	: AD80305_CONFIG_PART20_TX_ATT
project			: AD936x FPGA
version			: 0.1
author			: ShangHai HanRu
called by		: 
calling			:
description		: Set Tx Attenuation
...
begin
    ...
    case ( Current_state )
		...
		CONFIG_DATA:
			if ( ( Config_done_1byte == 1'b1 ) && ( Config_addr_1byte == 10'h07C ) ) 
...
begin
	if ( Reset == 1'b1 )
		...
		Config_addr_1byte <= 10'h077; 
	else if ( ( Current_state == CONFIG_DATA ) && ( Config_done_1byte == 1'b1 ) )
		case ( Config_addr_1byte )
            10'h077: Config_addr_1byte <= 10'h073;
            10'h073: Config_addr_1byte <= 10'h074;	
            10'h074: Config_addr_1byte <= 10'h075;	
            10'h075: Config_addr_1byte <= 10'h076;
            10'h076: Config_addr_1byte <= 10'h07C;
            default: Config_addr_1byte <= 10'h073;
		endcase
	else
		;
end
...
begin
	if ( Reset == 1'b1 )
        ...
        case ( Config_addr_1byte )
            10'h077: Config_data_wr_1byte <= 8'h40;                         //TxAtten word changes, the transmit power will update immediately
			10'h073: Config_data_wr_1byte <= Tx_gain_initial[7:0];			//* 10dB ATT, 0.25dB/LSB
			10'h074: Config_data_wr_1byte <= { 7'h0, Tx_gain_initial[8] };	
            10'h075: Config_data_wr_1byte <= Tx_gain_initial[7:0];			//* 10dB ATT, 0.25dB/LSB
            10'h076: Config_data_wr_1byte <= { 7'h0, Tx_gain_initial[8] };
			10'h07C: Config_data_wr_1byte <= 8'h40;
            default: Config_data_wr_1byte <= 8'h28;
        endcase
    else
        Config_data_wr_1byte <= 8'b0;
end
