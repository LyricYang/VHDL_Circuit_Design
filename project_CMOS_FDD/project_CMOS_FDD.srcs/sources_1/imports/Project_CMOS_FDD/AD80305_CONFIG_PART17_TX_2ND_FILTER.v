/************************************************************
module name 	: AD80305_CONFIG_PART17_TX_2ND_FILTER
project			: AD936x FPGA
version			: 0.1
author			: ShangHai HanRu
called by		: 
calling			:
description		: TX Secondary Filter Calibration Setup:  Real Bandwidth
...
begin
    ...
    case ( Current_state )
		...
		CONFIG_DATA:
			if ( ( Config_done_1byte == 1'b1 ) && ( Config_addr_1byte == 10'h0D0 ) ) 
...
begin
	if ( Reset == 1'b1 )
		...
		Config_addr_1byte <= 10'h0D2; 
	else if ( ( Current_state == CONFIG_DATA ) && ( Config_done_1byte == 1'b1 ) )
		case ( Config_addr_1byte )
			10'h0D2: Config_addr_1byte <= 10'h0D1;	
			10'h0D1: Config_addr_1byte <= 10'h0D0;	
			default: Config_addr_1byte <= 10'h0D2;
		endcase
	else
		;
end	
...
begin
	if ( Reset == 1'b1 )
		...
		case ( Config_addr_1byte )
			10'h0D2: Config_data_wr_1byte <= 8'h17;
			10'h0D1: Config_data_wr_1byte <= 8'h0C;	
			10'h0D0: Config_data_wr_1byte <= 8'h56;		  	
		  	default: Config_data_wr_1byte <= 8'h17;
		endcase
	else
		Config_data_wr_1byte <= 8'b0;
end
